      ...
      <div className="max-w-7xl mx-auto px-6 md:px-8 space-y-12">
        ...
        <div className="text-center md:text-left max-w-2xl space-y-3">
          <span className="text-xs text-accent-gold font-bold uppercase tracking-[4px] glow-gold flex items-center justify-center md:justify-start gap-1">
            <Sparkles className="w-4 h-4 text-accent-primary" /> Staging the Spectacle
          </span>
          <h1 className="text-4xl md:text-5xl font-black text-white tracking-tight font-display">
            Upcoming Experiences
          </h1>
          <p className="text-sm md:text-base text-text-muted leading-relaxed font-body">
        ...
        {/* Filter Toolbar Box */}
        <div className="bg-bg-surface border border-border-color p-6 md:p-8 rounded-[24px] shadow-xl space-y-6">
          <div className="flex flex-col md:flex-row gap-4 items-center justify-between">
            ...
            <div className="relative w-full md:max-w-sm">
              ...
              <input 
                ...
                className="w-full bg-bg-card border border-border-color text-xs text-white pl-12 pr-4 py-3.5 rounded-xl focus:border-accent-primary outline-none transition-all placeholder-gray-500 focus:bg-bg-main"
              />
            </div>
            ...
            <div className="flex flex-col sm:flex-row items-center justify-between md:justify-end gap-4 w-full md:w-auto">
              <div className="flex items-center gap-2.5 w-full sm:w-auto">
                ...
                <select 
                  ...
                  className="w-full sm:w-44 bg-bg-card border border-border-color text-xs text-white px-4 py-3.5 rounded-xl focus:border-accent-primary outline-none transition-all cursor-pointer font-semibold"
                >
              ...
              <div className="flex bg-bg-card border border-border-color rounded-xl p-1 shrink-0 self-end sm:self-auto">
                <button
        ...
        {loading ? (
                ...
                return (
                  <div
                    ...
                    className="relative bg-bg-card border border-border-color rounded-2xl overflow-hidden
                      flex flex-col md:flex-row gap-6 p-5 transition-all duration-300 group
                      hover:border-accent-primary/50 hover:shadow-2xl hover:shadow-accent-primary/15 animate-fade-in"
                  >
                    {/* Banner Image */}
                    <div className="w-full md:w-64 h-44 shrink-0 overflow-hidden relative rounded-xl bg-bg-surface">
                      <img
                    ...
                    <div className="flex flex-col justify-between flex-grow">
                      <div className="space-y-3">
                        ...
                        <h3 className="font-display text-xl text-text-main group-hover:text-accent-secondary transition-colors leading-snug">
                          {event.title}
                        </h3>
                      ...
                      <div className="pt-4 flex items-center justify-between border-t border-border-color/30 mt-4">
                        <Link
                          to={`/events/${event.id}`}
          ...
          /* Empty Search Result */
          <div className="text-center py-20 bg-bg-surface border border-border-color rounded-[32px] space-y-4 animate-fade-in">
            ...
            <p className="text-xs text-text-muted max-w-sm mx-auto">No events matching your search or filters could be verified. Clear your keywords or try another category.</p>
            <button 
              onClick={handleResetFilters}
              className="px-6 py-2.5 bg-bg-card hover:bg-accent-primary hover:border-accent-primary border border-border-color text-xs font-bold text-white rounded-xl uppercase tracking-wider transition-all cursor-pointer"
            >
              Clear Filters
            </button>
          </div>
